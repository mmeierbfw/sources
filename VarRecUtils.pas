...
// Creates a constant array from a stringarray
function ConstFromString(arr: array of string): TConstArray;
...
function ConstFromString(arr: array of string): TConstArray;
var ConstArray: TConstArray;
    cnt: integer;
    tmp: AnsiString;
begin
  SetLength(ConstArray, sizeof(arr));
  //Werte kopieren
  for cnt := Low(arr) to High(arr) - 1 do
  begin
    ConstArray[cnt].VType := vtAnsiString;
    tmp := ANsiString(arr[cnt]);
    ConstArray[cnt].VAnsiString := Pointer(tmp);
  end;
  Finalize(constarray);
  Result :=  ConstArray;
end;
